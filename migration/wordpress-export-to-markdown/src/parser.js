	...
	postTypes.forEach(postType => {
		const postsForType = getItemsOfType(data, postType)
			...
			.map(post => {
				...
				if (postType === 'scroll'){
					// Get the scrolls metadata from the post meta
					// For example - language_1 = Latin
					// <wp:postmeta>
					// 	<wp:meta_key><![CDATA[language_1]]></wp:meta_key>
					// 	<wp:meta_value><![CDATA[Latin]]></wp:meta_value>
					// </wp:postmeta>
					// Field: post_id, post_date, link, title, online_bibliography_record,
					// language_1, language_2, lib_lon, meta_title, date_quality,
					// length, repository_city, number_of_pieces, contents, completed,
					...
					let fields = ['online_bibliography_record', 'language_1', 'language_2', 'lib_lon', 'meta_title', 'date_quality', 'length', 'repository_city', 'number_of_pieces', 'contents', 'completed', 'repository_nation', 'orientation', 'description', 'prov_lon', 'date_start', 'prov_lat', 'width', 'repository', 'repository_name_and_city', 'shelfmark', 'provenance', 'lib_lat', 'online_images', '_scrolls_id'];
					let extraMeta = {}
					fields.forEach(field => {
						let value = getPostMeta(post, field);
						if (value !== undefined){
							extraMeta[field] = value;
						}
					});
					frontmatter = {...frontmatter, ...extraMeta};
				}
				return {
					meta: meta,
...
function getPostMeta(post, field){
	// console.log(field);
	...
	const value = postmeta ? postmeta.meta_value[0] : undefined;
	// console.log(value);
	return value;
}
